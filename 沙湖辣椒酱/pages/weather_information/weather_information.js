...
Page({
  data: {
    ...
    result_name: ['---',"日照时间", "昼夜温差",'温度','湿度','其他'],
    index: 0,
    index1: 0,
    index2: 0,
    index3: 0,
  ...
  goodsNameValue() {
    ...
    wx.request({
      url: getApp().globalData.url + '/api/basics/plot/base_area' + '?base_name=' + that.data.goodsName,
      method: 'GET',
      header: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'XMLHttpRequest',
  ...
  bingId(event) {
    this.setData({
      ...
      goodsName: event.currentTarget.dataset.id.base_name,
      array_goods_status: true,
      hiddenLoading: true,
    })
  },
  ...
  bindKeyInput(e) {
    ...
    if (e.detail.value !== '') {
      wx.request({
        url: getApp().globalData.url + '/api/basics/plot/base_area' + '?base_name=' + e.detail.value,
        method: 'GET',
        header: {
          'Content-Type': 'application/json',
          'X-Requested-With': 'XMLHttpRequest',
  ...
  formSubmit: function (e) {
    ...
    var collection_time = that.data.date2;
    var collection_detail = e.detail.value.purchase_quantity;
    var collection_item  = that.data.result[that.data.index4].num;
    ...
    wx.request({
      url: getApp().globalData.url + '/api/farming/weather',
      method: 'POST',
      data: {
